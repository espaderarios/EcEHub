...
export function simulate(input: {
  ...
  for (const part of input.parts) {
    if ((part.kind === "switch" || part.kind === "button") && part.props.closed) {
  ...
  const buzzers = input.parts.filter((p) => p.kind === "buzzer");
  const relays = input.parts.filter((p) => p.kind === "relay");

  const diodeOn = diodes.map(() => false);
  const ledOn = leds.map(() => false);
  const relayOn = relays.map(() => false);
  ...
  for (let iter = 0; iter < 8; iter++) {
    ...
    for (const part of input.parts) {
      ...
      if (part.kind === "inductor") {
        const a = pinHole(part, "a");
        const bpin = pinHole(part, "b");
        // In this DC teaching simulator an inductor settles to its winding
        // resistance. Transient current ramping is outside this solver.
        if (a && bpin) stampR(holeStrip(a), holeStrip(bpin), 6);
      }
      if (part.kind === "pot") {
        const a = pinHole(part, "a");
      ...
      if (part.kind === "buzzer") {
        const a = pinHole(part, "a");
        const bpin = pinHole(part, "b");
        if (a && bpin) stampR(holeStrip(a), holeStrip(bpin), 180);
      }
      if (part.kind === "mcu") {
        const vcc = pinHole(part, "vcc");
        const gnd = pinHole(part, "gnd");
        if (vcc && gnd) stampR(holeStrip(vcc), holeStrip(gnd), 5000);
      }
      if (part.kind === "lcd") {
        const vdd = pinHole(part, "vdd");
        const vss = pinHole(part, "vss");
        if (vdd && vss) stampR(holeStrip(vdd), holeStrip(vss), 350);
      }
    }

    relays.forEach((relay, ri) => {
      const coilA = pinHole(relay, "coilA");
      const coilB = pinHole(relay, "coilB");
      const common = pinHole(relay, "com");
      const normallyOpen = pinHole(relay, "no");
      if (coilA && coilB) stampR(holeStrip(coilA), holeStrip(coilB), 90);
      if (common && normallyOpen) {
        stampR(holeStrip(common), holeStrip(normallyOpen), relayOn[ri] ? 0.15 : 1e8);
      }
    });
    ...
    diodes.forEach((diode, di) => {
      const a = pinHole(diode, "a");
      const k = pinHole(diode, "k");
      if (!a || !k) return;
      const sa = uf.find(holeStrip(a));
      const sk = uf.find(holeStrip(k));
      const i = indexOf(sa);
      ...
      if (diodeOn[di]) {
        ...
        if (i >= 0) b[i] += g * DIODE_VF;
        if (j >= 0) b[j] -= g * DIODE_VF;
      } else {
        stampR(holeStrip(a), holeStrip(k), 5e7);
      }
    });
    ...
    diodes.forEach((diode, di) => {
      ...
      const va = voltageOf(uf.find(holeStrip(a)), gndStrip, idx, x);
      const vk = voltageOf(uf.find(holeStrip(k)), gndStrip, idx, x);
      const next = va - vk > DIODE_VF * 0.65;
      if (next !== diodeOn[di]) {
        diodeOn[di] = next;
        changed = true;
      }
    ...
    relays.forEach((relay, ri) => {
      const coilA = pinHole(relay, "coilA");
      const coilB = pinHole(relay, "coilB");
      if (!coilA || !coilB) return;
      const coilVoltage = Math.abs(
        voltageOf(uf.find(holeStrip(coilA)), gndStrip, idx, x) -
          voltageOf(uf.find(holeStrip(coilB)), gndStrip, idx, x),
      );
      const next = coilVoltage >= 3;
      if (next !== relayOn[ri]) {
        relayOn[ri] = next;
        changed = true;
      }
  ...
  const diodeStates: SimResult["diodes"] = {};
  diodes.forEach((diode, di) => {
    const a = pinHole(diode, "a");
    const k = pinHole(diode, "k");
    if (!a || !k) return;
    const voltage = (voltages[a] ?? 0) - (voltages[k] ?? 0);
    const current = diodeOn[di] ? Math.max(0, (voltage - DIODE_VF) / DIODE_RD) : 0;
    if (current * 1000 > DIODE_MAX_MA) {
      warnings.push(`Diode ${diode.props.label ?? diode.id} is overcurrent (${(current * 1000).toFixed(0)} mA). Add a series resistor.`);
    }
    diodeStates[diode.id] = {
      id: diode.id,
      on: diodeOn[di] && current > 0.00001,
      current,
  ...
  const buzzerStates: SimResult["buzzers"] = {};
  buzzers.forEach((buzzer) => {
    const voltage = Math.abs(vAt(buzzer, "a") - vAt(buzzer, "b"));
    const current = voltage / 180;
    buzzerStates[buzzer.id] = {
      id: buzzer.id,
      on: voltage >= 2.5,
      current,
      loudness: Math.max(0, Math.min(1, (voltage - 2.5) / 3)),
    };
  });

  const relayStates: SimResult["relays"] = {};
  relays.forEach((relay, ri) => {
    const coilVoltage = Math.abs(vAt(relay, "coilA") - vAt(relay, "coilB"));
