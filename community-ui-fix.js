...
(() => {
   ...
   * LIVE PEOPLE SEARCH
   *
   * search-fix.js currently derives People from public
   * flashcard authors. That misses users who have no public
   * flashcard set. The Worker already has /api/search, which
   * returns the complete public user search independently.
   * ========================================================== */
  ...
  function injectPeopleResults(users, query) {
    ...
    if (cleanUsers.length) {
      ...
      const html = `
        <div class="global-search-group community-ui-people-group">
          <div class="global-search-group-title">People</div>
          ${cleanUsers.slice(0, 5).map(userRow).join('')}
        </div>`;
    ...
    const sectionHtml = `
      <section class="global-search-section community-ui-people-section" data-search-section="users">
        <div class="global-search-section-head">
          <div>
            <span class="global-search-section-icon">◉</span>
            <h2>People</h2>
          </div>
          <span>${cleanUsers.length}</span>
        </div>
        <div class="global-search-results-grid">
          ${cleanUsers.slice(0, 12).map(userCard).join('')}
        </div>
      </section>`;
    ...
    if (section) {
      ...
      filters.insertAdjacentHTML('afterend', sectionHtml);
    } else {
      page.insertAdjacentHTML('beforeend', sectionHtml);
    }

    /* If the underlying search returned zero results, replace the
       empty-state message because People now supplied results. */
  ...
  /* ==========================================================
   * COMMUNITY FLASHCARD EDIT MODAL
   * ========================================================== */
  ...
  function cardEditor(card, index) {
    return `
      <div class="community-edit-card" data-card-index="${index}">
        <div class="community-edit-card-head">
          <strong>Card ${index + 1}</strong>
          <button type="button" class="btn danger community-edit-remove-card">Remove</button>
        </div>
        <label>
          Question
          <textarea class="community-edit-question" rows="2" maxlength="2000">${esc(card.question)}</textarea>
        </label>
        <label>
          Answer
          <textarea class="community-edit-answer" rows="3" maxlength="4000">${esc(card.answer)}</textarea>
        </label>
      </div>`;
  }
  ...
  function injectModalStyles() {
    ...
    style.textContent = `
      ...
      .community-edit-modal-header h2{margin:0 0 5px}
      .community-edit-modal-header p{margin:0;opacity:.7}
      .community-edit-modal-close{border:0;background:transparent;font-size:28px;cursor:pointer;padding:0 4px;color:inherit}
      .community-edit-modal-body{padding:22px 24px;overflow:auto}
      .community-edit-grid{display:grid;grid-template-columns:1fr 1fr;gap:14px}
      .community-edit-field{display:flex;flex-direction:column;gap:7px;margin-bottom:14px}
      .community-edit-field.full{grid-column:1/-1}
      .community-edit-field label{font-weight:700}
      ...
      .community-edit-cards{display:flex;flex-direction:column;gap:12px;margin-top:18px}
      .community-edit-card{border:1px solid rgba(127,127,127,.22);border-radius:14px;padding:14px;background:rgba(127,127,127,.035)}
      .community-edit-card-head{display:flex;justify-content:space-between;align-items:center;margin-bottom:12px}
      .community-edit-card label{display:flex;flex-direction:column;gap:6px;margin-top:10px;font-weight:600}
      .community-edit-card textarea{width:100%;box-sizing:border-box;border:1px solid rgba(127,127,127,.3);border-radius:9px;padding:9px;background:transparent;color:inherit;font:inherit}
      .community-edit-add-card{margin-top:12px}
      .community-edit-modal-footer{display:flex;justify-content:flex-end;gap:10px;padding:16px 24px;border-top:1px solid rgba(127,127,127,.18)}
      .community-edit-status{margin-top:12px;min-height:20px;color:#b42318}
  ...
  async function openCommunitySetEditModal(setId) {
    ...
    backdrop.innerHTML = `
      <div class="community-edit-modal" role="dialog" aria-modal="true" aria-labelledby="community-edit-title">
        <div class="community-edit-modal-header">
          <div>
            <h2 id="community-edit-title">Edit community flashcards</h2>
            <p>Update the set, cards, and whether other students can discover it.</p>
          </div>
          <button type="button" class="community-edit-modal-close" data-community-edit-close aria-label="Close">×</button>
        </div>

        <div class="community-edit-modal-body">
          <div class="community-edit-grid">
            <div class="community-edit-field">
              <label for="community-edit-title-input">Title</label>
              <input id="community-edit-title-input" maxlength="120" value="${esc(set.title)}">
            </div>
            <div class="community-edit-field">
              <label for="community-edit-subject-input">Subject</label>
              <input id="community-edit-subject-input" maxlength="120" value="${esc(set.subject)}">
            </div>
            <div class="community-edit-field full">
              <label for="community-edit-description-input">Description</label>
              <textarea id="community-edit-description-input" maxlength="1000" rows="3">${esc(set.description)}</textarea>
            </div>
            <div class="community-edit-field">
              <label for="community-edit-visibility-input">Visibility</label>
              <select id="community-edit-visibility-input">
                <option value="public" ${set.visibility === 'public' ? 'selected' : ''}>Public — searchable by everyone</option>
                <option value="private" ${set.visibility === 'private' ? 'selected' : ''}>Private — only you can access it</option>
              </select>
            </div>
          </div>

          <div class="community-edit-cards" id="community-edit-cards">
            ${cards.map(cardEditor).join('')}
          </div>
        ...
        <div class="community-edit-modal-footer">
          <button type="button" class="btn" data-community-edit-close>Cancel</button>
          <button type="button" class="btn primary" data-community-edit-save>Save changes</button>
        </div>
      </div>`;
    ...
    backdrop.querySelectorAll('[data-community-edit-close]').forEach(button => {
    ...
    backdrop.addEventListener('click', event => {
      ...
      if (container.querySelectorAll('.community-edit-card').length <= 1) {
        ...
        status.textContent = 'A flashcard set must contain at least one card.';
        return;
      }
      card?.remove();
      renumberEditCards(container);
    ...
    backdrop.querySelector('[data-community-edit-save]')?.addEventListener('click', async () => {
      ...
      if (!updatedCards.length) {
        status.textContent = 'Add at least one flashcard.';
        return;
      }

      const invalid = updatedCards.findIndex(card => !card.question || !card.answer);
      if (invalid !== -1) {
        status.textContent = `Card ${invalid + 1} needs both a question and an answer.`;
        return;
      }
      ...
      try {
        ...
        if (typeof window.loadCommunityFlashcards === 'function') {
          try {
            window.communityFlashcardsLoaded = false;
          } catch {}
        }
  ...
  function installEditHandler() {
    document.addEventListener('click', event => {
      ...
      /* Capture/bubble ordering is not guaranteed by the app's
         delegated handler, so this prevents the old no-op action
         from reaching action(). */
